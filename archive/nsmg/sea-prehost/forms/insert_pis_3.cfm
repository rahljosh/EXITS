...
<Cfquery name="lastname" datasource="mysql">
select smg_hosts.familylastname, smg_hosts.city, smg_hosts.state, smg_states.statename, smg_hosts.hostid
from smg_hosts
left join smg_states on smg_states.state = smg_hosts.state
where hostid = #url.hostid#
</Cfquery>
<cfoutput>
<cfmail to='brian@student-management.com' cc='josh@pokytrails.com' from='hostfamapp@exitsapplication.com' subject='Pre-Host Family Application'>
The #lastname.familylastname# family from  #lastname.city#  #lastname.statename# has submitted an application to host. 

View the profile: http://www.student-management.com/nsmg/index.cfm?curdoc=host_fam_info&hostid=#hostid#

OR 

Under Host Family, Search for Host ID: #hostid#

This profile is not assigned to a region.  Once you've assigned to the appropriate regrion, forward this email to the appropriate contact person. 

Regards
</cfmail>
</cfoutput>
<cflocation url="../index.cfm?curdoc=forms/thanks" addtoken="no">
