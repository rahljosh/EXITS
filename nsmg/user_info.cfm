...
<cfcase value="set_default_uar">
    ...
    <cfquery datasource="#application.dsn#">
        ...
        SET default_access = 0
        WHERE id IN (#valueList(get_all_access.id)#)
    </cfquery>
...
<cfcase value="verify_info">
	<cfquery datasource="#application.dsn#">
        UPDATE smg_users
        SET last_verification = <cfqueryparam cfsqltype="cf_sql_date" value="#now()#">
...
<cfquery name="rep_info" datasource="#application.dsn#">
	SELECT smg_users.*, smg_countrylist.countryname, smg_insurance_type.type
	FROM smg_users
    LEFT JOIN smg_countrylist ON smg_users.country = smg_countrylist.countryid
	LEFT JOIN smg_insurance_type ON smg_users.insurance_typeid = smg_insurance_type.insutypeid
	WHERE userid = <cfqueryparam cfsqltype="cf_sql_integer" value="#url.userid#">
</cfquery>
...
<cfquery name="get_paperwork" datasource="#application.dsn#">
	SELECT p.paperworkid, p.userid, p.seasonid, p.ar_info_sheet, p.ar_ref_quest1, p.ar_ref_quest2, p.ar_cbc_auth_form, p.ar_agreement, p.ar_training, s.season
	FROM smg_users_paperwork p

	LEFT JOIN smg_seasons s ON s.seasonid = p.seasonid
	WHERE p.userid = <cfqueryparam cfsqltype="cf_sql_integer" value="#rep_info.userid#">
	<cfif client.companyid eq 10>
    and p.fk_companyid = 10
    </cfif>
			...
			<table width=100% cellpadding=10 cellspacing=0 border=0 class="section" >
                ...
                <cfif uar_usertype EQ 8>
						...
						<table width=245 cellpadding=0 cellspacing=0 border=0 bgcolor="##ffffff" align="left">
							...
							<tr>
								...
								<td width=226 style="padding:5px;">
                                    ...
                                    <cfif billing_phone NEQ ''>Phone: #billing_phone#<br></cfif>
                                    <cfif billing_fax NEQ ''>Fax: #billing_fax#<br></cfif>
                                    <cfif billing_email NEQ ''>Email: <a href="mailto:#billing_email#">#billing_email#</a><br></cfif>	
								</td>
								<td width=6 style="border-right: 1px solid ##557aa7;">&nbsp;</td>
							</tr>
							<tr valign="bottom">
					...
					<td>
                        ...
                        <cfif client.usertype LTE 4 OR client.usertype LT uar_usertype>
	...
	<tr>
		...
		<td width="50%" valign="top">
			...
			<cfquery name="get_cbc_user" datasource="#application.dsn#">
				SELECT cbcid, date_authorized , date_sent, date_received, requestid, smg_users_cbc.seasonid, flagcbc, smg_seasons.season, batchid
				FROM smg_users_cbc
