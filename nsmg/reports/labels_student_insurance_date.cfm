		...
		<div class="Section1">
			...
			<cfquery name="get_students" datasource="MySql"> 			
				...
				WHERE s.active = '1'
					...
					AND ( <cfloop list="#form.insurance#" index='insu'>
							 s.insurance = '#insu#'
							<cfif insu is #ListLast(form.insurance)#><Cfelse>or</cfif>
						</cfloop> )						
					<!---
					<cfif form.date1 NEQ '' AND form.date2 NEQ ''>
						AND (s.date_host_fam_approved between #CreateODBCDateTime(form.date1)# and #CreateODBCDateTime(DateAdd('d', 1, form.date2))#)
					</cfif>	
					--->
				ORDER BY s.insurance, s.sevis_batchid, u.businessname, s.firstname
			</cfquery>
			<!---
