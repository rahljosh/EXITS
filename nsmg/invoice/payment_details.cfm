...
SELECT sc.invoiceid, spc.*, SUM(amountapplied) AS amountapplied, spr.date_applied AS date, spr.paymenttype
...
<!--- <Cfloop query="payment_details"> --->
	...
	select sum(spc.amountapplied) as amountapplied
	from smg_payment_charges spc
	LEFT JOIN smg_payment_received spr ON spr.paymentid = spc.paymentid
	where spr.paymentref  = '#url.ref#'
	</cfquery>
...
<cfquery name="qAgentName" datasource="MySQL">
SELECT businessname
FROM smg_users
...
<table cellpadding="4" cellspacing="0" class="thin-border" width="530">
	<th><div class="application_section_header">Payment Details for <cfoutput>#qAgentName.businessname#</cfoutput></div></th>
</table><br />

<table cellpadding="4" cellspacing="0" class="thin-border" width="530">
	<th colspan=4>Payment Reference Number: #url.ref#</th>
	<tr bgcolor="##CCCCCC">
	...
	<!--- <cfloop query="payment_details"> --->
		...
		<cfloop query="get_charge_payments">
			...
			<cfquery name="get_charge_payment" datasource="MySQL">
				select distinct smg_charges.invoiceid
				from smg_charges 
				left join smg_payment_charges on smg_charges.chargeid = smg_payment_charges.chargeid
				where smg_payment_charges.paymentid = #payment_details.paymentid#
			</cfquery>
			<tr bgcolor="<cfif currentrow mod 2 EQ 0>white<cfelse>##D9D8E2</cfif>">
				<td>&nbsp;</td>
				<td colspan="2">Above amount applied to invoice(s): ###invoiceid#<br></td>
				<td>&nbsp;</td>
			</tr>
		</cfloop>
	<!--- </cfloop> --->
