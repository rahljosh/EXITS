...
<!--- all these events are type template --->
	...
	<cfif NOT len(attributes.name)>
		...
		<cfoutput><cfloop from="1" to="#arraylen(links)#" index="i">
			...
			<li<cfif listfind(selected,links[i].owner) OR (structkeyexists(links[i],"eventName") AND listfind(selected,links[i].eventName))> class="current"</cfif>><a href="#address#"<cfif structkeyexists(links[i],"icon")> style="background-image:url(#links[i].icon#);"</cfif>>#links[i].title#</a></li>	
		</cfloop></cfoutput>
