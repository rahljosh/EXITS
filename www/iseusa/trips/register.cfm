...
insert into student_tours (studentid, tripid, cc, cc_year, nationality, stunationality, person1, person2, person3, med, billingAddress, billingCity, billingState, billingZip, billingcountry, cc_month, date, ip)
			values (#form.ret_studentid#, #x#, '#EncrpytedCC#', #form.cc_year#, '#form.nationality#', '#form.stunationality#', '#form.person1#', '#form.person2#', '#form.person3#', '#form.med#',  '#form.billingAddress#', '#form.billingCity#', '#form.billingState#', '#form.billingZip#', '#form.billingcountry#', #form.cc_month#, now(), '#cgi.REMOTE_ADDR#')
</cfquery>
...
from student_tours
    ...
    <cfset sib_EncrpytedCC = encrypt(form.sibling_cc, 'BB9ztVL+zrYqeWEq1UALSj4pkc4vZLyR', "desede", "hex")>
            <cfquery datasource="#APPLICATION.DSN.Source#">
        insert into student_tours_siblings (fk_studentid, mastertripid,siblingid, tripid, nameOnCard, cc, cc_year, billingAddress, billingCity, billingState, billingZip, billingcountry, cc_month)
                    values (#form.ret_studentid#, #tripid.mastertripid#, #i#, #x#, '#form.sibling_nameoncard#','#sib_EncrpytedCC#', #form.sibling_cc_year#,  '#form.sibling_billingAddress#', '#form.sibling_billingCity#', '#form.sibling_billingState#', '#form.sibling_billingZip#', '#form.sibling_billingcountry#', #form.sibling_cc_month#)
        </cfquery>
    </cfloop>
</cfif>
</Cfloop>
...
<Br />
<cfif isDefined('form.host_siblings')>
<h2>HOST SIBLING IS REGISTERED AS WELL</h2>
</cfif>
			...
			<cfif stuInfo.recordcount eq 1 and stuInfo.hostid gt 0>            
                ...
                <Table width=100% cellspacing=0 cellpadding=2 class="border">
                    ...
                    <Cfset kidsage = #DateDiff('yyyy', kids.birthdate, now())#>
                        <Cfif ListFind('15,16,17,18', '#kidsage#')>
                        <cfset oktogo = 1>
						<cfset showsibcc = #showsibcc# + #oktogo#>
                         </Cfif>
