				...
				$tmp_dir_path = FILESYSTEM_PATH . "tempdir";
 				...
 				foreach ($o_ticket->threads as $one_thread)
 				{
 					...
 					foreach ($one_thread->ptr->file_attachments as $attm)
 					{
 						...
 						{
							$tmp_name = tempnam(realpath("./tempdir"),"cerb"); 
//							$tmp_name = tempnam($tmp_dir_path, "cerb");
		...
		case "clone":
		{
			$sql = sprintf("SELECT t.ticket_id, t.ticket_subject, t.is_deleted, t.is_closed, t.is_waiting_on_customer, t.ticket_date, unix_timestamp(t.ticket_due) as mktime_due, t.ticket_queue_id, t.ticket_priority, th.thread_address_id, ad.address_address, q.queue_name, t.min_thread_id, t.max_thread_id, t.ticket_reopenings, t.ticket_time_worked, t.last_reply_by_agent, t.opened_by_address_id " .
				"FROM ticket t  " .
			...
			$sql = sprintf("INSERT INTO ticket(ticket_subject,ticket_date,ticket_priority,ticket_queue_id,ticket_reopenings,min_thread_id,max_thread_id,ticket_mask,ticket_time_worked,last_reply_by_agent,opened_by_address_id) ".
				"VALUES(%s,%s,%d,%d,%d,%d,%d,%s,%d,%d,%d)",
			...
			if($cerberus_db->num_rows($result) > 0)
			{
				...
				{
					$sql = sprintf("INSERT INTO thread(ticket_id,thread_message_id,thread_address_id,thread_type,thread_date,thread_time_worked,is_agent_message) ".
						"VALUES(%d,%s,%d,%s,%s,%d,%d)",
						$clone_id,
						$cerberus_db->escape($ar["thread_message_id"]),
