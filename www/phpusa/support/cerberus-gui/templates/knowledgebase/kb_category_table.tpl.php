...
{foreach from=$ancestors item=ans key=ans_id}
	{if $ans_id}
		<a href="{"knowledgebase.php?root="|cat:$ans_id|cer_href}" class="cer_knowledgebase_link">{$kb->flat_categories.$ans_id->name}</a> : 
	{else}
		...
		<td width="50%" valign="top">
			<table cellpadding="1" cellspacing="0" border="0" style="padding-right:2px;border-right:1px dashed #dddddd">
				...
				{foreach from=$kb_root->children item=category key=category_id name=categories}
					<tr>
						<td colspan="2"><img src="includes/images/icone/16x16/folder.gif" align="absmiddle"><img src="includes/images/spacer.gif" width="5" height="1" align="absmiddle"><a href="{"knowledgebase.php?root="|cat:$category->id|cer_href}" class="kb_category"><b>{$category->name}</b></a> ({$category->hits})</td>
					</tr>
					...
					<tr>
						<td width="100%"><img src="includes/images/icone/16x16/document.gif" align="absmiddle" alt="article"> <a href="javascript:popupResource({$article->id});" class="cer_knowledgebase_link">{$article->name}</a></td>
