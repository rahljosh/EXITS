    ...
    <tr>
      ...
      {if $catId > 0}
      	...
      	{if $category->level == 1 && $smarty.foreach.manageDescendents.iteration > 2}
      		<br>
      	{/if}
		<img src="includes/images/icone/16x16/folder.gif" align="absmiddle"><img src="includes/images/spacer.gif" width="5" height="1"><a href="javascript:;" onclick="getFnrCategoryEdit({$catId});">{$category->name}</a> ({$category->hits})<br>
	  {/if}
      {/foreach}
      </div>
</td>
