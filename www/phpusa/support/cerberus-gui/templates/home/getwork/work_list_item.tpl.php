     ...
     <td>
	   	...
	   	{if $show_take && $acl->has_priv($smarty.const.PRIV_TICKET_CHANGE)}
	   		{if !isset($ticket->flags.$user_id)}
	   			<a href="javascript:void(0);" onclick="preWorkTake({$ticket->id});" class="link_navmenu"><img alt="Flag" src="includes/images/icone/16x16/flag_green.gif" width="16" height="16" border="0" align="middle" style="padding-right:2px;" onmouseover="getWorkToolTip({$ticket->id},'Flag this ticket in My Tickets');" onmouseout="getWorkToolTip({$ticket->id},'');"></a>
	   		{else}
