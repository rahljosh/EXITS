...
switch ($mode)
{
	case "preferences":
	{
		...
		if($cerberus_db->num_rows($result) == 0)
			{
			$sql = sprintf("INSERT INTO user_prefs (user_id,ticket_order,user_language,signature_pos,signature_autoinsert,keyboard_shortcuts,gmt_offset) ".
	   	...
	   	$sql = sprintf("UPDATE user_prefs ".
	   		"SET ticket_order=%d, signature_pos=%d, signature_autoinsert=%d, keyboard_shortcuts=%d, gmt_offset=%s",
	   			$ticket_order,
	   			$signature_pos,
	   			$signature_autoinsert,
	   			$keyboard_shortcuts,
