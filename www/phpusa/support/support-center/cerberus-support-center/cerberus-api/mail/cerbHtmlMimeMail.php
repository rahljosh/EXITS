<?php
/*!
	\file cerbHtmlMimeMail.php
	\brief PEAR Mime Mail E-mail Class.

	\note PEAR Core classes adapted by WebGroup Media LLC
	\whole freaking thing changed to cerbHtmlMimeMail (from htmlMimeMail)
	\because of conflict with modernbill's use of htmlMimeMail class.
...
*/
	...
	/*
	 * [JAS]: Make sure we aren't sending reply/watcher mail to any queue addresses.
	 */
//	function _sanitizeEnvelope($addys) {
//		/* @var $queues cer_QueueHandler */
//		/* @var $db cer_Database */
//
//		if(defined("CERB_IGNORE_LOOP_PROTECTION") && CERB_IGNORE_LOOP_PROTECTION)
//			return $addys;
//		
//		static $blockedAddys;
//		
//		if(empty($blockedAddys)) {
//			$queues = cer_QueueHandler::getInstance();
//			$blockedAddys = $queues->getQueueAddresses();
//		}
//
//		if(is_array($addys))
//		foreach($addys as $idx => $addy) {
//			$checkAddy = $addy->mailbox . '@' . $addy->host;
//			if(isset($blockedAddys[$checkAddy])) {
//				unset($addys[$idx]);
//			}
//		}
//		
//		// [JAS]: Return only unblocked addresses
//		return $addys;
//	}
			...
			case 'mail':
				...
				foreach ($this->headers as $name => $value) {
					if($name == 'Cc' || $name == 'Bcc') {
						...
						$headers[] = $name . ': ' . $this->_encodeHeader($addresses, $this->build_params['head_charset']);
					} else {
...
//				}
			...
			case 'smtp':
				require_once(FILESYSTEM_PATH . 'cerberus-api/mail/smtp.php');
				...
				foreach ($this->headers as $name => $value) {
					if ($name == 'Cc' || $name == 'Bcc') {
						$addresses = Mail_RFC822::parseAddressList($value, $this->smtp_params['helo'], null, false);
						$this->headers[$name] = implode(', ', $addresses); // [JAS]: Rewrite header
