...
class CER_PARSER_TICKET
{
	...
	function _get_ticket_spam_probability()
	{
		...
		$sql = sprintf("SELECT th.thread_subject FROM thread th ".
...
//		else
	...
	function add_ticket_thread(&$email,$thread_type,$is_new=false,$auto_cc_requester=true)
	{
		...
		if(!empty($email->headers->_to_raw)) {
			$to_list = $this->db->escape($email->headers->_to_raw);
		...
		if (!empty($email->headers->date)) {
			$thread_date = date("Y-m-d H:i:s", strtotime(@$email->headers->date));
			$thread_received = date("Y-m-d H:i:s", strtotime(@$email->headers->date));
		} else {
		...
		sprintf("VALUES (%d,%s,%d,%s,%s,%s,%s,%s,%s,%s,%d)",
		$this->ticket_id,
...
//echo $sql;exit();
		...
		if($is_new) {
			$sql = sprintf("UPDATE ticket SET min_thread_id = %d, max_thread_id = %d, last_reply_by_agent = %d, opened_by_address_id = %d ".
			"WHERE ticket_id = %d",
				$thread_id,
				$thread_id,
				$is_agent_reply,
