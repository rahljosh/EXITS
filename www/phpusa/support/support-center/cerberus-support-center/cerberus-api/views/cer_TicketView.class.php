...
//			$use_tags = true; // ack
		...
		"INNER JOIN address ad ON (ad.address_id = th.thread_address_id) ".
		(($tag_sql) ? $tag_sql : "") .
//		(($team_sql) ? $team_sql : "") .
		(($use_flags) ? ((($this->params['criteria']['flag']['flag_mode']) ? "INNER":"LEFT") . " JOIN ticket_flags_to_agents fta ON (fta.ticket_id = t.ticket_id) ") : "") .
		(($agent_sql) ? $agent_sql : "") .
		"%s ".
...
//		echo "<HR>" . $sql . "<BR>";
		...
		if($this->db->num_rows($res)) {
			while($row = $this->db->fetch_row($res)) {
				$t_ids[$row["ticket_id"]] = $row["address_id"];
//				$r_ids[$row["thread_address_id"]] = $row["thread_address_id"];
		...
		$sql = sprintf("SELECT t.ticket_id, t.ticket_subject, t.ticket_priority, t.ticket_spam_trained, t.last_reply_by_agent, ".
			"t.ticket_spam_probability, th.thread_date, thr.thread_received, t.is_closed, t.is_deleted, t.is_waiting_on_customer, t.ticket_due, ".
			...
			"LEFT JOIN thread thr ON (t.min_thread_id = thr.thread_id) ".
			"LEFT JOIN thread th ON (t.max_thread_id = th.thread_id) ".
			"LEFT JOIN address a ON (a.address_id = th.thread_address_id) ".
			"LEFT JOIN address ad ON (ad.address_id = t.opened_by_address_id) ".
			"%s ". // use company?
			"%s ". // custom field join?
			"WHERE 1 ".
...
//		}
	...
	function _buildCreatedDateSearch() {
		...
		if(isset($this->params['criteria']['created'])) {
			...
			$date_sql = sprintf(" AND thr.thread_date BETWEEN %s AND %s",
	...
	function _buildUpdatedDateSearch() {
		...
		if(isset($this->params['criteria']['last_updated'])) {
			...
			$date_sql = sprintf(" AND th.thread_date BETWEEN %s AND %s",
				$this->db->escape($f_string),
				$this->db->escape($t_string)
			);
		}
