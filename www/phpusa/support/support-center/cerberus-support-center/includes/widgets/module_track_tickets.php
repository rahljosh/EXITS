...
if(!empty($ticket)) {
	...
	// [TAR]: Added to view Open vs Closed Tickets
		if($view_type == "Closed"){
			...
			$ticket_display_label = "Closed Ticket History";
			$change_view_type = "Open";
		} else {
			$track_ticket_type = "AND t.is_closed = 0 AND t.is_deleted = 0 ";
			$ticket_display_label = "Open Ticket History";
...
if($cer_session->is_logged_in) {
	...
	if(!empty($ticket_id)) {
		...
		$sql = sprintf("SELECT t.ticket_id, t.ticket_mask, t.ticket_subject, t.is_closed, t.is_deleted, t.is_waiting_on_customer, t.ticket_due, t.ticket_date, t.ticket_priority, t.ticket_time_worked, thr.is_agent_message, th.thread_date, a.address_address as requester_address, a.address_id as requester_address_id, q.queue_name, q.queue_prefix, q.queue_reply_to ".
				"FROM (ticket t, requestor r, thread thr, thread th, address a, queue q) ".
				"WHERE t.ticket_id = r.ticket_id AND thr.thread_id = t.min_thread_id AND th.thread_id = t.max_thread_id AND a.address_id = r.address_id AND t.ticket_queue_id = q.queue_id ".
				(!empty($track_ticket_type) ? $track_ticket_type : "") .
				"AND r.address_id IN (%s) AND t.ticket_id = %d ".
		...
		switch($form_submit) {
			case "add_reply": {
				...
				if(empty($track_ticket) || empty($reply_body) || empty($reply_from)) {
					require ("track_tickets/box_track_ticket_view.php");
					break;
				}
				...
				$body = "Client IP: " . $_SERVER['REMOTE_ADDR'] . "\r\n\r\n" .
					$content;
	...
	elseif(!empty($ticket)) {
		...
		require ("track_tickets/box_track_open_tickets.php");
	}
}
else {
