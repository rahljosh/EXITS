...
<cfif NOT IsDefined('url.change')>
	...
	<cftransaction action="BEGIN" isolation="SERIALIZABLE">
		...
		<cfquery name="place_area" datasource="mysql">
		...
		<cfif check_history.recordcount EQ '0' AND boarding_school.boarding_school EQ '1' AND form.arearepid NEQ '0' AND get_student_unqid.placerepid NEQ '0'>
			<cfquery name="create_original_placement" datasource="mysql">
				...
				values('0', '#get_student_unqid.studentid#', '#get_program_info.schoolid#', 'yes',
				 #CreateODBCDateTime(now())#, '#form.arearepid#', '#get_program_info.placerepid#', '#client.userid#', 'Original Placement')
			</cfquery>
			PRIMEIRO<br />
		<cfelseif check_history.recordcount EQ '0' AND get_student_unqid.hostid NEQ '0' AND form.arearepid NEQ '0' AND get_student_unqid.placerepid NEQ '0'>
			<cfquery name="create_original_placement" datasource="mysql">
				INSERT INTO smg_hosthistory	(hostid, studentid, schoolid, original_place, dateofchange, arearepid, placerepid, changedby, reason)
...
<!--- UPDATE/CHANGE CURRENT SUPERVISING --->
<cfelse>
