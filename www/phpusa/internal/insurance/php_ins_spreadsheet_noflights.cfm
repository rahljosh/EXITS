...
select distinct php_students_in_program.studentid, 
<!----stu.firstname, stu.familylastname, stu.dob, stu.schoolid, 
smg_insurance_type.insutypeid, smg_insurance_type.type,  smg_insurance_codes.policycode, smg_insurance_type.insutypeid, 
smg_programs.startdate, smg_programs.enddate, smg_programs.programname,   smg_programs.insurance_startdate, smg_programs.insurance_enddate, smg_programs.enddate AS program_end_Date,
----> php_students_in_program.canceldate,
php_students_in_program.programid 
FROM php_students_in_program 

LEFT JOIN smg_students stu ON stu.studentid = php_students_in_program.studentid
LEFT JOIN smg_programs ON smg_programs.programid = php_students_in_program.programid 
...
WHERE php_students_in_program.canceldate IS NULL 
AND php_students_in_program.programid= 230
AND php_students_in_program.studentid NOT IN (SELECT studentid from php_insurance_batch where studentid = php_students_in_program.studentid)
...
<cfquery name="stu_ins" datasource="mysql">
select distinct php_students_in_program.studentid, php_students_in_program.canceldate, php_students_in_program.programid,
smg_students.firstname, smg_students.familylastname, smg_students.dob, smg_insurance_type.type,
smg_programs.startdate, smg_programs.enddate
FROM php_students_in_program 
...
WHERE php_students_in_program.canceldate IS NULL 
AND (php_students_in_program.programid = 230 or php_students_in_program.programid = 262)
...
<cfquery name="stu_ins1" datasource="mysql">
	...
	where
	smg_programs.active = 1 <!----and stu.studentid NOT IN (select studentid  from smg_insurance_batch2 where type = 'N')
	and php.companyid = 6---->
    stu_programid = 
	and php.active = 1
...
<cfset xlsfilename = 'php_no_flights_#DateFormat(now(),'mmddyyyy')#'>
...
<!-----save xls to server---->
	...
	---->
					...
					<cfquery name="insert_student_id" datasource="mysql">
						insert into php_insurance_batch (studentid, file,type, date, startdate, enddate)
