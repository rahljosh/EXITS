...
<script language="javascript">
	...
	var populateVerificationList = function(verList) { 
		...
		for(i=0;i<verList.DATA.length;i++) { 
			...
			var ds2019_startdate = verList.DATA[i][verList.COLUMNS.findIdx('DS2019_STARTDATE')];
			...
			var start = new Date(ds2019_startdate);
			var startInMillis = start.getTime();
			var millisInDay = 86400000;
			// They are non compliant if the ds2019 began more than 10 days ago.
			if (nowInMillis - startInMillis < (30*millisInDay)) {
				nonCompliant = 0;
			}
			// They are in a warning state if the ds2019 began more than 5 days ago.
