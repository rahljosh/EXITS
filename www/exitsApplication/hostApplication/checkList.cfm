...
//]]>
    ...
    and hostID = #APPLICATION.CFC.SESSION.getHostSession().ID#
    </cfquery>
...
<cfif checkSig.recordcount gt 0 AND #Right(cgi.HTTP_REFERER, '9')# eq 'checklist'>
    ...
    <cfquery name="hostName" datasource="#APPLICATION.DSN.Source#">
...
<cfquery name="cl" datasource="#APPLICATION.DSN.Source#">
SELECT * 
FROM smg_hosts
WHERE hostID = #APPLICATION.CFC.SESSION.getHostSession().ID# 
</cfquery>
...
<!------------------------>
<!----Family Members---->
<cfquery name="familyMembers" datasource="#APPLICATION.DSN.Source#">
	SELECT *
	FROM smg_host_children
	WHERE hostID = #VAL(cl.hostID)#
</cfquery>
...
<cfquery name="hostPicCat" datasource="#APPLICATION.DSN.Source#">
	select *
    from smg_host_pic_cat
    <!---Don't include Other description---->
    where catID != 7
</Cfquery>
<cfquery name="hostPics" datasource="#APPLICATION.DSN.Source#">
	select *
    from smg_host_picture_album
    where fk_hostID = #VAL(cl.hostID)#
</Cfquery>
...
select *
from smg_schools
where schoolid = #VAL(cl.schoolid)#
</Cfquery>
<cfscript>
...
<!----References---->
<cfquery name="references" datasource="#APPLICATION.DSN.Source#">
	SELECT *
	FROM smg_family_references
	WHERE referencefor = #VAL(cl.hostID)#
</cfquery>
