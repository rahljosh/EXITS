...
<cfset repsonse2 = #PreserveSingleQuotes(response)#>
		 ...
		 insert into smg_prquestion_details (response, stuid, report_number, yn, userid, companyid, date, question_number, uniqueid, month_of_report)
		 	values( '#repsonse2#', #url.stuid#, #report_number#, '#Evaluate("FORM." & x & "_yn")#', #client.userid#, #client.companyid#, #rightnow#, #Evaluate("FORM." & x & "_question_number")#,  '#uniqueid#', #form.month_of_report#)
		 </Cfquery>
...
<cfelse>
		...
		<cfquery name="approve_r1" datasource="MySQL">
		update smg_document_tracking
