...
<cfoutput>
	...
	<cfloop query="get_students">
		...
		<!---
		<!--- PUSH START DATE TO A DATE FURTHER --->
		<cfif get_history.new_date EQ '2006-07-20'>
			<cfquery name="insurance_correction" datasource="MySql">
				...
				VALUES
					...
					#CreateODBCDate(get_students.dob)#, '#get_students.countrycode#', '2006-08-15',
					#CreateODBCDate(get_students.insurance_enddate)#, '#get_history.org_code#', '#get_history.policy_code#', 'correction');	
			</cfquery>
			Correction file created for #firstname# #familylastname# (###studentid#) - New Date 2006-08-15 - Insurance Start - #DateFormat(get_history.new_date, 'mm/dd/yyyy')#<br>
		</cfif>
		--->
		...
		<cfif get_flight.recordcount AND get_history.sent_to_caremed NEQ ''>
			...
			<cfif get_flight.dep_date NEQ get_history.new_date AND get_history.new_date NEQ #DateFormat(now(), 'mm/dd/yyyy')#>
					...
					<cfquery name="insurance_correction" datasource="MySql">
						...
						VALUES
							...
							<cfif get_flight.dep_date LT now() AND get_history.new_date GT now()>
								#CreateODBCDate(now())#,
							<cfelseif get_history.new_date GT #DateFormat(now(), 'mm/dd/yyyy')#>
								#CreateODBCDate(get_flight.dep_date)#,
							<cfelse>
								#CreateODBCDate(now())#,
							</cfif>
							#CreateODBCDate(get_students.insurance_enddate)#, '#get_history.org_code#', '#get_history.policy_code#', 'correction')
					...
					<cfif get_flight.dep_date LT now() AND get_history.new_date GT now()><b>STUDENT HAS ARRIVED</b><br></cfif>
