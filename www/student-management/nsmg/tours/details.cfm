...
where mastertripid = #url.id#
    ...
    <cfif details.cc is not ''>
     <cfset sib_decrpytedCC =decrypt(sib_details.cc, 'BB9ztVL+zrYqeWEq1UALSj4pkc4vZLyR', "desede", "hex")>
    <cfelse>
     <cfset sib_decrpytedCC = 'No CC on file due to Google.'>
    </cfif>
   	<tr>
    	<Td>CC Number:</Td><td><input type="text" value="#sib_decrpytedCC#" name="sib_cc" /></td>
    </tr>
    <tr>
    ...
    <table width=98% cellpadding = 4 cellspacing = 0>
    	<tr>
        	<Td>Resend Permission Forms</Td><td>
            <cfif details.hostid eq 0>No Host Family Assigned - Permission Form is based on Host Info.<cfelse>
            <form method="post" action="details.cfm?student=#details.studentid#&id=#url.id#&message1=resent"><cfif details.verified is not ''><input name="" type="image" src="../pics/send-email.gif" /> <cfif isDefined('url.message1')>#url.message1#</cfif><input type="hidden" name="resendpermision" /><cfelse>Student not verified</cfif></form></cfif></td>
        </tr>
        <Tr>
