...
<cfif isDefined('form.deleteSeason')>
	<cfquery name="deleteSeason" datasource="#application.dsn#">
    	DELETE FROM
        	smg_users_paperwork
    	WHERE
        	seasonid = <cfqueryparam value="#form.deleteSeason#" cfsqltype="cf_sql_integer" maxlength="6">
    	AND
        	userid = <cfqueryparam value="#form.userid#" cfsqltype="cf_sql_integer" maxlength="6">
    </cfquery>
</cfif>
    ...
    <cftransaction action="begin" isolation="SERIALIZABLE">	  
		<cfloop From = "1" To = "#form.count#" Index = "x">
            <cfquery name="update_paperwork" datasource="MySQL">
                ...
                WHERE
                	paperworkid = '#form["paperworkid_" & x]#'
                LIMIT 1
            </cfquery>	
		</cfloop>
		<cfscript>
...
<cfif val(get_paperwork.reviewAcct)>
    ...
    <cfsavecontent variable="programEmailMessage">
    	<cfoutput>				
        	...
        	<br /><br />
          	<a href="#client.exits_url#/nsmg/index.cfm?curdoc=user_info&userid=#qGetUserInfo.userid#">
            	View #qGetUserInfo.firstname#
				<cfif Right(#qGetUserInfo.firstname#, 1) is 's'>
                	'
				<cfelse>
                	's
				</cfif> 
