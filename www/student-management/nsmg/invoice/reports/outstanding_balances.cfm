...
<cfloop query="get_int_reps">
	<!----ISE Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="total_Due_ise" datasource="MySQL">
			...
			<cfquery name="received_payments_ise" datasource="mysql">
			...
			<!----Sum of credits---->
			<cfquery name="sum_credits_ise" datasource="MySQL">
			...
			<cfquery name="overpayment_credit_ise" datasource="MySQL">
	...
	<!----INTO Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="total_Due_into" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 2
			and agentid = #userid#
			...
			<!----sum of received payments---->
			<cfquery name="received_payments_into" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 2
			and agentid = #userid#
			...
			<!----Sum of credits---->
			<cfquery name="sum_credits_into" datasource="MySQL">
			SELECT sum(amount) AS credit_amount, sum(amount_applied) AS credit_applied, sum(amount)-sum(amount_applied) AS totalcredit
			from smg_credit
			where companyid = 2
			and agentid = #userid# and active = 1
			...
			<cfif sum_credits_into.totalcredit is ''>
				<cfset sum_credits_into.totalcredit = 0>
			</cfif>
			
			<!----Sum of Overpayment Credits---->
			<cfquery name="overpayment_credit_into" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
			where agentid = #userid# and payref <> '' and active = 0
			and companyid = 2
			...
			<cfset into_bal = #total_Due_into.amount_due# - #received_payments_into.amount_paid# - #sum_credits_into.totalcredit# + #overpayment_credit_into.overpayment_amount# >


	<!----ASAI Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="total_Due_asa" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 3
			and agentid = #userid#
			...
			<!----sum of received payments---->
			<cfquery name="received_payments_asa" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 3
			and agentid = #userid#
			...
			<!----Sum of credits---->
			<cfquery name="sum_credits_asa" datasource="MySQL">
			SELECT sum(amount) AS credit_amount, sum(amount_applied) AS credit_applied, sum(amount)-sum(amount_applied) AS totalcredit
			from smg_credit
			where companyid = 3
			and agentid = #userid# and active = 1
			...
			<cfif sum_credits_asa.totalcredit is ''>
				<cfset sum_credits_asa.totalcredit = 0>
			</cfif>
			
			<!----Sum of Overpayment Credits---->
			<cfquery name="overpayment_credit_asa" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
			where agentid = #userid# and payref <> '' and active = 0
			and companyid = 3
			...
			<cfset asa_bal = #total_Due_asa.amount_due# - #received_payments_asa.amount_paid# - #sum_credits_asa.totalcredit# + #overpayment_credit_asa.overpayment_amount#>
			
	<!----DMD Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="total_Due_dmd" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 4
			and agentid = #userid#
			...
			<!----sum of received payments---->
			<cfquery name="received_payments_dmd" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 4
			and agentid = #userid#
			...
			<!----Sum of credits---->
			<cfquery name="sum_credits_dmd" datasource="MySQL">
			SELECT sum(amount) AS credit_amount, sum(amount_applied) AS credit_applied, sum(amount)-sum(amount_applied) AS totalcredit
			from smg_credit
			where companyid = 4
			and agentid = #userid# and active = 1
			...
			<cfif sum_credits_dmd.totalcredit is ''>
				<cfset sum_credits_dmd.totalcredit = 0>
			</cfif>
			
			<!----Sum of Overpayment Credits---->
			<cfquery name="overpayment_credit_dmd" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
			where agentid = #userid# and payref <> '' and active = 0
			and companyid = 4
			...
			<cfset dmd_bal = #total_Due_dmd.amount_due# - #received_payments_dmd.amount_paid# - #sum_credits_dmd.totalcredit# + #overpayment_credit_dmd.overpayment_amount#>
			
	<!----SMG Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="total_Due_smg" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 5
			and agentid = #userid#
			...
			<!----sum of received payments---->
			<cfquery name="received_payments_smg" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 5
			and agentid = #userid#
			...
			<!----Sum of credits---->
			<cfquery name="sum_credits_smg" datasource="MySQL">
			SELECT sum(amount) AS credit_amount, sum(amount_applied) AS credit_applied, sum(amount)-sum(amount_applied) AS totalcredit
			from smg_credit
			where companyid = 5
			and agentid = #userid# and active = 1
			...
			<cfif sum_credits_smg.totalcredit is ''>
			<cfset sum_credits_smg.totalcredit = 0>
			</cfif>
			
			<!----Sum of Overpayment Credits---->
			<cfquery name="overpayment_credit_smg" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
			where agentid = #userid# and payref <> '' and active = 0
			and companyid = 5
			...
			<cfset smg_bal = #total_Due_smg.amount_due# - #received_payments_smg.amount_paid# - #sum_credits_smg.totalcredit# + #overpayment_credit_smg.overpayment_amount#>
...
<!----Grand Totals Due---->
	<!----ISE Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="gtotal_Due_ise" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 1
			and (date between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----sum of received payments---->
			<cfquery name="greceived_payments_ise" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 1
			and (date_applied between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----Sum of credits---->
			<cfquery name="gsum_credits_ise" datasource="MySQL">
			...
			<!----Sum of Overpayments---->
			<cfquery name="goverpayment_credit_ise" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
	...
	<!----INTO Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="gtotal_Due_into" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 2
			and (date between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----sum of received payments---->
			<cfquery name="greceived_payments_into" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 2
			and (date_applied between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----Sum of credits---->
			<cfquery name="gsum_credits_into" datasource="MySQL">
			select sum(amount) as credit_amount
			from smg_credit
			...
			<!----Sum of Overpayments---->
			<cfquery name="goverpayment_credit_into" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
			where payref <> '' and active = 0
			and companyid = 2
	...
	<!----ASAI Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="gtotal_Due_asa" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 3
			and (date between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----sum of received payments---->
			<cfquery name="greceived_payments_asa" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 3
			and (date_applied between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----Sum of credits---->
			<cfquery name="gsum_credits_asa" datasource="MySQL">
			select sum(amount) as credit_amount
			from smg_credit
			...
			<!----Sum of Overpayments---->
			<cfquery name="goverpayment_credit_asa" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
			where payref <> '' and active = 0
			and companyid = 3
	...
	<!----DMD Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="gtotal_Due_dmd" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 4
			and (date between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----sum of received payments---->
			<cfquery name="greceived_payments_dmd" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 4
			and (date_applied between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----Sum of credits---->
			<cfquery name="gsum_credits_dmd" datasource="MySQL">
			select sum(amount) as credit_amount
			from smg_credit
			...
			<!----Sum of Overpayments---->
			<cfquery name="goverpayment_credit_dmd" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
			where payref <> '' and active = 0
			and companyid = 4
	...
	<!----SMG Numbers---->			
			<!----sum of amount due---->
			<Cfquery name="gtotal_Due_smg" datasource="MySQL">
			select sum(amount_due) as amount_due
			from smg_charges
			where companyid = 5
			and (date between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----sum of received payments---->
			<cfquery name="greceived_payments_smg" datasource="mysql">
			select sum(totalreceived) as amount_paid
			from smg_payment_received
			where companyid = 5
			and (date_applied between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#)
			...
			<!----Sum of credits---->
			<cfquery name="gsum_credits_smg" datasource="MySQL">
			select sum(amount) as credit_amount
			from smg_credit
			...
			<!----Sum of Overpayments---->
			<cfquery name="goverpayment_credit_smg" datasource="MySQL">
			select sum(amount) as overpayment_amount
			from smg_credit
			where payref <> '' and active = 0
			and companyid = 5
