...
<cfif not IsDefined("form.filename") or form.filename is ''>
	...
	<tr><th bgcolor="#ededed">AMEND DATES UPDATE</th></tr>
	<tr><td>
		<cfform method="post" action="?curdoc=sevis/amend_dates_extract" enctype="multipart/form-data">
...
<cfelse>
	...
	<cffile action="read" file="#AppPath.sevis##get_company.companyshort_nocolor#/amend/#form.filename#" variable="myxml">
	...
	<tr><th colspan="3" bgcolor="#ededed">AMEND DATES UPDATE &nbsp; - &nbsp;BATCH RESULTS</th></tr>
	<cfoutput>
	...
	<cfloop index="i" from = "1" to = #numItems#>
		...
		<cfif #mydoc.TransactionLog.BatchDetail.Process.RECORD[i].Result.XmlAttributes.Status# is 'true'>
			...
			<cfquery name="update_ds2019" datasource="MySql"> <!--- UPDATE SEVIS ACTIVATED TO 0 --->
				UPDATE smg_students
				SET sevis_amend_dates ='0'
				WHERE studentid = '#Right(batchid, 4)#'
			</cfquery>				
		<cfelse> <!--- ERROR - DS 2019 WAS NOT CREATED --->
			<cfoutput>
			...
			<!--- UPDATE SEVIS BATCH ID TO 0 STUDENT WILL COME BACK TO THE XML NEXT TIME --->
	...
	<br>
	<table width="100%" align="center" frame="box">
	<tr><th colspan="3" bgcolor="#ededed">AMEND DATES UPDATE &nbsp; - &nbsp; ERRORS</th></tr>
	<cfoutput>
	<tr><td colspan="3" align="center" bgcolor="ededed">File Name &nbsp; : &nbsp; &nbsp; <b>#filename#</b></td></tr>
	<tr><td colspan="3" align="center" bgcolor="ededed">Batch ID &nbsp; : &nbsp; &nbsp; <b>#batchid#</b></td></tr>
	<tr><td colspan="3"> &nbsp; Total Requested in this Batch &nbsp;  : &nbsp; &nbsp; <b>#totalrequest#</b></td></tr>
