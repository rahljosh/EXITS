...
<cfloop query="get_region">
	...
	<cfloop query="list_repid">
		...
		<Cfquery name="get_students_region" datasource="MySQL">
			...
			st.statename, smg_regions.regionname
        
			from smg_students s
			LEFT OUTER JOIN smg_countrylist c ON s.countryresident = c.countryid
			LEFT JOIN smg_hosts h ON s.hostid = h.hostid
			LEFT JOIN smg_states st ON st.id = s.state_guarantee
			LEFT JOIN smg_regions ON smg_regions.regionid = s.regionalguarantee
