...
<cfquery name="get_host" datasource="MySQL">
	SELECT s.studentid, s.hostid,
			h.familylastname, h.fatherfirstname, h.motherfirstname, h.address, h.address2, h.city, h.state, h.zip, h.phone
	FROM smg_students s
...
<table width=680 border=0 align="center" bgcolor="FFFFFF">	
	...
	<tr><td class="style3"><b>Host Family.: </b></td>
		<td class="style3">
		<cfif get_host.fatherfirstname is not ''>Mr. #get_host.fatherfirstname#</cfif>
		<cfif get_host.fatherfirstname is not '' and get_host.motherfirstname is not ''> and </cfif>
		<cfif get_host.motherfirstname is not ''>Mrs. #get_host.motherfirstname#</cfif>
		#get_host.familylastname# (#get_host.hostid#) &nbsp &nbsp Phone : &nbsp #get_host.phone#</td></tr>	
