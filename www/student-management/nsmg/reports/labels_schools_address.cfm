<cfsilent>
	...
	<cfquery name="get_schools" datasource="MySql"> 
        SELECT 
            ...
            sc.address2 AS sc_address2, 
            sc.city AS sc_city, 
            sc.state AS sc_state, 
            sc.zip AS sc_zip, 
            sc.phone AS sc_phone,
            ar.userid, 
            ar.lastname AS ar_lastname, 
            ar.firstname AS ar_firstname, 
            ar.address AS ar_address, 
            ar.address2 AS ar_address2, 
            ar.city AS ar_city, 
            ar.state AS ar_state, 
            ar.zip AS ar_zip, 
            ar.phone AS ar_phone,
            c.countryname,
            p.programname, 
            p.startdate, 
            p.enddate
        ...
        INNER JOIN 
            smg_users ar ON s.arearepid = ar.userid
        INNER JOIN 
        ...
        <cfif LEN(FORM.date1) AND LEN(FORM.date2)>
            AND
            (
                s.dateplaced BETWEEN
                    #CreateODBCDateTime(FORM.date1)# 
                AND 
