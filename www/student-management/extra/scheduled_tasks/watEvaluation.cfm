...
<cfscript>
	...
	vEvaluationEmailBody = vEvaluationEmailBody &
	...
	vEvaluationEmailBody = vEvaluationEmailBody &
		...
		text-align:center;'><a href='http://www.csb-usa.com/evaluation/evaluation.cfm'>
	...
	For ( i=1;i LTE qEvaluation2.Recordcount; i=i+1 ) {
		...
		if ( IsValid("email", qEvaluation2.email[i]) ) {
			vEmailTo = vEmailTo & qEvaluation2.email[i];		
		}
		if ( LEN(vEmailTo) ) {
			APPLICATION.CFC.EMAIL.sendEmail(
				emailFrom=vEmailFrom,
	...
	For ( i=1;i LTE qEvaluation3.Recordcount; i=i+1 ) {
		...
		if ( IsValid("email", qEvaluation3.email[i]) ) {
			vEmailTo = vEmailTo & qEvaluation3.email[i];		
		}
		if ( LEN(vEmailTo) ) {
			APPLICATION.CFC.EMAIL.sendEmail(
				emailFrom=vEmailFrom,
	...
	For ( i=1;i LTE qEvaluation4.Recordcount; i=i+1 ) {
		...
		if ( IsValid("email", qEvaluation4.email[i]) ) {
			vEmailTo = vEmailTo & qEvaluation4.email[i];		
		}
		if ( LEN(vEmailTo) ) {
			APPLICATION.CFC.EMAIL.sendEmail(
				emailFrom=vEmailFrom,
