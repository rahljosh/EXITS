<!--- ------------------------------------------------------------------------- ----
	...
	Date:		May 16, 2012
	Desc:		Missing check-in / validation date. Allows the user to mark validation as received.
...
<script language="javascript">
	...
	var getVerificationList = function() { 
		...
		c.getMissingValidation(intlRep);
	} 
	...
	var populateVerificationList = function(verList) { 
		...
		for(i=0;i<verList.DATA.length;i++) { 
			...
			var tableBody = '';	
				...
				tableBody += '<td align="center" class="style5"><a href="javascript:setValidationReceived(' + candidateID + ');" class="style4">[Received]</a></td>';
				tableBody += '</tr>';
			// Append table rows
	...
	var setValidationReceived = function(candidateID) {
		
		// Create an instance of the proxy. 
		var c = new candidate();

		// Setting a callback handler for the proxy automatically makes the proxy's calls asynchronous. 
		c.setCallbackHandler(validationReceived(candidateID)); 
		c.setErrorHandler(myErrorHandler); 
		
		// This time, pass the intlRep ID to the getVerificationList CFC function. 
		c.confirmValidationReceived(candidateID);
		
	}
	
	var validationReceived = function(candidateID) {
		
		// Set up page message, fade in and fade out after 2 seconds
		$("#candidateDetailMessage").text("Validation for candidate #" + candidateID + " received").fadeIn().fadeOut(3000);
...
<cfoutput>
	...
	<!--- Table Header --->    
    <gui:tableHeader
        tableTitle="Missing Check-in / Validation Date"
    />
