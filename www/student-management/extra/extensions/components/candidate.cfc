...
<cfcomponent 
    ...
    <!------------------------------------------------------------
		WAT - Missing Validation Tool 
	------------------------------------------------------------->
	<cffunction name="getMissingValidation" access="remote" returnFormat="json" output="false" hint="Returns verification report list in Json format">
    	<cfargument name="intRep" default="0" hint="International Representative is not required">
        <cfargument name="receivedDate" default="" hint="Filter by verification received date">

        <cfquery 
			name="qGetMissingValidation" 
			datasource="#APPLICATION.DSN.Source#">
                SELECT
					ec.candidateID,
                    ec.firstName,
		...
		<cfreturn qGetMissingValidation>
	</cffunction>
    
    <cffunction name="confirmValidationReceived" access="remote" returntype="void" hint="Sets verification_received field as received.">
        <cfargument name="candidateID" required="yes" hint="candidateID is required">
        ...
        <cfquery 
			datasource="#APPLICATION.DSN.Source#">
                UPDATE
					extra_candidates
				SET
                    watDateCheckedIn = <cfqueryparam cfsqltype="cf_sql_timestamp" value="#now()#">
                WHERE
                    candidateID = <cfqueryparam cfsqltype="cf_sql_integer" value="#ARGUMENTS.candidateID#">
		</cfquery>
    ...
    <!------------------------------------------------------------
		End of WAT - Missing Validation Tool 
	------------------------------------------------------------->
