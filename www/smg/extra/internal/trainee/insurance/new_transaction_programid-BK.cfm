...
<cfquery name="get_candidates" datasource="MySQL">
	SELECT c.candidateid, c.firstname, c.lastname, c.sex, c.dob, c.arrivaldate, c.enddate,
	...
	WHERE c.active = '1'
		...
		<!--- AND c.verification_received IS NOT NULL --->
...
<table border="1" cellpadding="3" cellspacing="0">
	...
	<cfloop query="get_candidates">
		<cfif arrivaldate NEQ '' AND enddate NEQ ''>
			<tr>
				<td>#orgcode#</td>
				<td><cfif extra_insurance_typeid NEQ '0'>#policycode#<cfelse>Missing Policy Type - #businessname#</cfif></td>
				<td>New App</td>
				<td>#lastname#</td>
				<td>#FirstName#</td>
				<td>#DateFormat(dob, 'dd/mmm/yyyy')#</td>
				<td>#sex#</td>
				<td><cfif arrivaldate NEQ ''>#DateFormat(arrivaldate, 'dd/mmm/yyyy')#<cfelse>Missing</cfif></td>
				<td><cfif enddate NEQ ''>#DateFormat(enddate, 'dd/mmm/yyyy')#<cfelse>Missing</cfif></td>
				<td>#countrycode#</td>
				<td>US</td>
				<td>INT</td>
			</tr>
			<!---<cfif policycode NEQ '' AND arrivaldate NEQ '' AND enddate NEQ ''> 
				...
				<cfquery name="insert_history" datasource="MySql">
					...
					VALUES
						...
						#CreateODBCDate(arrivaldate)#, #CreateODBCDate(enddate)#, '#orgcode#', '#policycode#',
