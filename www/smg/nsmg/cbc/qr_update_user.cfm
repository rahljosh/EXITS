...
<cfif form.repid NEQ '' OR form.repid NEQ '0' OR form.newrepid NEQ '' OR form.repid NEQ '0'>
	...
	<cfquery name="check_active_old" datasource="mysql">
		select active
		from smg_users
		where userid =#form.repid#
	</cfquery>
	
	<cfquery name="check_active_new" datasource="mysql">
		select active
		from smg_users
	...
	<cfquery name="list_arearep" datasource="MySql">
		SELECT studentid
		FROM smg_students
		WHERE arearepid = #current_id#
	</cfquery>
	
	<cfquery name="update_arearep" datasource="MySql">
		UPDATE smg_students
		SET arearepid = #current_id#
		where arearepid = #updated_id#
	</cfquery>

	<cfquery name="list_placerep" datasource="MySql">
		SELECT studentid
		FROM smg_students
		WHERE placerepid = '#current_id#'
	</cfquery>
	
	<cfquery name="update_placerep" datasource="MySql">
		UPDATE smg_students
	...
	<cfquery name="update_payments" datasource="MySql">
 	...
 	<cfquery name="old_region_assignments" datasource="MySql">
		SELECT regionid
		FROM user_access_rights
		WHERE userid = #current_id#
	</cfquery>
	 
	 <cfquery name="new_region_assignments" datasource="MySql">
		SELECT regionid
		FROM user_access_rights
		WHERE userid = #updated_id#
	</cfquery>	 
	 
	<cfquery name="update_regions" datasource="mysql">
		update user_access_rights
		set userid = #current_id#
		where userid = #updated_id#
	</cfquery>
	
	<cfquery name="update_progress_reports" datasource="mysql">
