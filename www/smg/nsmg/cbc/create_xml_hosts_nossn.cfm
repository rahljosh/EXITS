...
<cfif form.usertype NEQ 'member'>
	...
	<cfquery name="get_cbc_hosts" datasource="MySql">
		SELECT DISTINCT 
        	cbc.cbcfamid, cbc.hostid, cbc.cbc_type, cbc.date_authorized, cbc.date_sent, cbc.date_received,
		...
		WHERE cbc.date_sent IS NULL 
			AND requestid = ''
            <cfif NOT ListFind("1,2,3,4", CLIENT.companyID)>
            AND 
			...
			<cfif form.usertype EQ 'father'>
				AND (fatherssn = '' OR fatherssn IS NULL)
			<cfelseif form.usertype EQ 'mother'>
				AND (motherssn = '' OR motherssn IS NULL)
			</cfif>
		LIMIT 20
	</cfquery>
