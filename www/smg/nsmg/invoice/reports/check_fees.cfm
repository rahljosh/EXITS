...
<cfloop query="get_agent">
	<cfquery name="get_students" datasource="MySql">
		SELECT s.studentid, s.firstname, s.familylastname, u.businessname, p.programname
		FROM smg_students s
		INNER JOIN smg_programs p ON s.programid = p.programid
		INNER JOIN smg_users u ON s.intrep = u.userid
		WHERE p.seasonid = '#form.seasonid#'
	...
	<cfif get_students.recordcount NEQ 0>
		...
		<cfelse>
			...
			<tr bgcolor="#iif(get_students.currentrow MOD 2 ,DE("ffffe6") ,DE("white") )#">
				<td colspan="2">&nbsp; &nbsp; &nbsp;#get_students.firstname# #get_students.familylastname# (###get_students.studentid#) #get_students.programname#</td>
			</tr>
			</cfloop>
		</cfif>
	</cfif>
