...
<cfquery name="list_schools" datasource="mysql">
SELECT * 
FROM smg_schools
Order by schoolname
</cfquery>
...
<cfloop query="list_schools">
	<cfoutput>
	<cfquery name="duplicates" datasource="mysql">
	SELECT *
	from smg_schools
	...
	<cfif duplicates.recordcount GT 1>
		...
		<cfoutput>
			<cfquery name="check_link_student" datasource="mysql">
			Select studentid
			FROM smg_students
			where schoolid = #duplicates.schoolid#
			</cfquery>	
			<cfquery name="check_link_host" datasource="mysql">
			Select hostid
			FROM smg_hosts
			where schoolid = #duplicates.schoolid#
			</cfquery>
