...
<cfquery name="insert_message" datasource="caseusa">
	insert into smg_news_messages
		(message, messagetype,startdate,expires,details,companyid, lowest_level <cfif isDefined('form.additional_file')>, additional_file</cfif>)
	values
		(<cfqueryparam value="#form.message#" cfsqltype="cf_sql_char">, 
		'#form.messagetype#', '#form.startdate#', '#form.expires#', 
		<cfqueryparam value="#form.details#" cfsqltype="cf_sql_char">, 
		'#form.companyid#', #form.lowest_level# <cfif isDefined('form.additional_file')>,'#form.additional_file#'</cfif>)
</cfquery>
