<cfquery name="current_alerts" datasource="caseusa">
select * 
from smg_news_messages
where  (expires > #now()#)
order by messagetype
</cfquery>
<Cfquery name="alert_types" datasource="caseusa">
select messagetype
from smg_news_messages
</Cfquery>
...
<cfoutput query="current_Alerts">
	...
	<tr>
		<td colspan=6>
		To add a new message fill out the following form.<br>  Alert messages should pertain to problems - appear in red area at top of page
<br> Update messages should pertain to new or changed items - appear in green area at top of page<br>
		...
		<table bgcolor="##ffffe6" width=100% border=0>
			...
			<tr>
				<td>Company:</td><td>
					...
					<cfloop list=#client.companies# index=i>
					<cfquery name="companies_available" datasource="caseusa">
					select companyid,companyshort
					from smg_companies
					where companyid = #i#
					</cfquery>
					<cfoutput><option value=#companies_available.companyid#>#companies_available.companyshort#</option></cfoutput>
				</cfloop>
				</select>
				</td>
			</tr>
			...
			<tr>
				<td valign="top">Lowest User View:</td><td>
					...
					- 'All CASE Users' does not include Int. Reps. 
				</td>
			</tr>
			<tr>
