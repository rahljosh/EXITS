...
<cfloop list='1,2,3,4,5' index="x">
		...
		<cfquery name="total_charges" datasource="caseusa">
		...
		<cfquery name="total_paid" datasource="caseusa">
...
<cfquery name="ttotal_charges" datasource="caseusa">
		select sum(amount_due) as amount_due
		from smg_charges 
		...
		<cfquery name="ttotal_paid" datasource="caseusa">
		select sum(totalreceived) as total_paid
		from smg_payment_received
		where agentid = #userid#
		and (date between #CreateODBCDateTime(form.beg_Date)# and #CreateODBCDateTime(form.end_date)#) and companyid = #x#
