...
<cfif not isDefined('form.end_date')>
	<cfset form.end_date = '08/31/2009'>
</cfif>
...
<cfset agent_company_paid = #agent_company_paid# + #total_invoice_amount_received#>
	...
	<td>#LSCurrencyFormat(agent_company_bal,'local')#</td><cfif #x# eq 10>
    
    <td>#LSCurrencyFormat(agent_company_total_charged,'local')#</td>
    <Cfset companygt = #companygt# + #agent_company_total_charged#>
    <td>#LSCurrencyFormat(companygt,'local')#</td>
...
<tr>
	<td></td><td></td><td></td><td><cfoutput>Total Billed = #LSCurrencyFormat(companygt,'local')#</cfoutput></td>
</tr>
</table>
