...
<Table width=100% cellpadding=4 cellspacing=0 class="thin-border">
	<tr bgcolor="000066">
		...
		<cfquery name="compshort" datasource="caseusa">
		select companyshort from smg_companies
		where companyid = #comp#
		</cfquery>
		<td><font color="white">Out Standing</td>
		<cfset "smg_comp_total_due_#comp#" = 0>
		</cfloop>
		
		<td><font color="white">Total Billed</td>
...
<cfloop list=#form.agents# index='agnt'>
	...
	<cfset total_charge_comp =0>
			<cfloop list=#form.companies# index='comp'>
					...
					<!----Loop to catch selected agents---->
						...
						<!----Gets the amount that needs to be paid---->
							<cfquery name="get_students_charges" datasource="caseusa">
		...
		<td>#LSCurrencyFormat(total_due_company,'local')#</td>
		<cfset gt_total_billed = #gt_total_billed# + #total_due_company#>
		</tr>
</cfloop>
<td> <strong>Totals</td>
<Cfset gtdue = 0>
...
<td><strong> #LSCurrencyFormat(gt_total_billed,'local')#</td>
