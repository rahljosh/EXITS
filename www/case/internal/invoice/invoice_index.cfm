							...
							<table width = 100%>
								...
								<tr>
									<td valign="top"><!----Reports---->
										...
										<a href="invoice/reports/outstanding_balances5.cfm">Invoice Balance Report</a><br>
									</td>
									<td valign="top"><!----Tools---->
...
<table width=100% cellpadding=0 cellspacing=0 border=0 height=24>
						<tr valign=middle height=24>
							<td height=24 width=13 background="pics/header_leftcap.gif">&nbsp;</td>
							<td width=26 background="pics/header_background.gif"><img src="pics/$.gif"><img src="pics/$.gif"><img src="pics/$.gif"></td>
							<td background="pics/header_background.gif"><h2>All International Agents</td><td background="pics/header_background.gif" align="right" ><h2>Totals for <cfoutput>#original_company_info.companyshort#</cfoutput></td>
							<td width=17 background="pics/header_rightcap.gif">&nbsp;</td>
						</tr>
					</table>
...
---->
			...
			<!----Sum of credits---->
			<cfquery name="sum_credits" datasource="caseusa">
			select sum(amount) as credit_Amount
			from smg_credit
...
<cfquery name="agents_Students" datasource="caseusa">
...
<cfset balance_due = #total_due.amount_due# - #total_credit#- #total_received.total_received# + #overpayment_credit.overpayment_amount#>
		...
		<tr>
		<cfquery name="sum_due" datasource="caseusa">
		select sum(amount_due)as total_amount_due from 
		smg_charges 
		</cfquery>
		<cfif sum_due.total_amount_due is ''>
		<cfset sum_due.total_amount_due = 0>
		</cfif>
		<cfquery name="sum_received" datasource="caseusa">
		select sum(totalreceived)as total_received from 
		smg_payment_received 
		</cfquery>
		<cfif sum_received.total_received is ''>
		<cfset sum_due.total_received = 0>
		</cfif>
		<cfoutput>
