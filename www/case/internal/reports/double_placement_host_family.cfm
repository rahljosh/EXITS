...
<cfquery name="get_student_info" datasource="caseusa">
	SELECT s.familylastname, s.address, s.address2, s.city, s.zip, s.hostid, s.doubleplace, s.familylastname, s.firstname, s.ds2019_no,
...
<cfquery name="double_placement" datasource="caseusa">
	SELECT s.firstname, s.familylastname, s.countryresident, c.countryname, s.schoolid, s.hostid, s.ds2019_no,
			p.programname, p.startdate
	FROM smg_students s
	INNER JOIN smg_countrylist c ON c.countryid = s.countryresident
	INNER JOIN smg_programs p ON p.programid = s.programid
...
<cfquery name="get_hf" datasource="caseusa">
...
<tr><td class="style1"><div align="justify">
	...
	<p>Please sign on the lines below and return to #companyshort.companyshort# so that we may finalize the placement.</p>
	</div>
</td></tr>
</table>
...
<tr><td class="style1" colspan="2">
	<div align="justify">
		By signing this, we agree that the two students listed above will live with the above host
		 family while participating in the #companyshort.companyshort# Program.</div></td></tr>
