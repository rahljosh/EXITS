<cfif not IsDefined('form.programid')><br>
...
<cfquery name="get_program" datasource="caseusa">
	...
	WHERE 	<cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop>
</cfquery>

<cfquery name="get_stu_in_prog" datasource="caseusa">
	SELECT	 count(studentid) as count_stu
	FROM 	smg_students
	WHERE active = '1' AND onhold_approved <= '4'
		  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
</cfquery>

<cfquery name="get_stu_placed" datasource="caseusa">
	SELECT	 count(studentid) as count_stu
	FROM 	smg_students
	WHERE active = '1' and hostid != '0' AND `host_fam_approved` < '5' AND onhold_approved <= '4'
		  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
</cfquery>
...
<tr><td align="left">
	...
	<font face="" color="Gray">&nbsp &nbsp <cfif client.companyid is 5>#companyshort# &nbsp; &nbsp</cfif></font><b>#programname# &nbsp - #programtype# &nbsp;</b><br>
	</cfloop>
...
<cfquery name="unplaced_placed" datasource="caseusa">
	SELECT 'Unplaced', count( studentid ) AS result 
	FROM smg_students
	WHERE hostid = 0 and active = 1 AND onhold_approved <= 4
		  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> ) 
	OR hostid != 0 and active = 1 AND `host_fam_approved` > 4
		  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> ) 	  
	UNION 
	SELECT 'Placed', count( studentid ) AS result 
	FROM smg_students
	WHERE hostid != 0 and active = 1 AND `host_fam_approved` < 5 AND onhold_approved <= 4
		  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
</cfquery>
<table align="center" width="90%" cellpadding=6 cellspacing="0" frame="box">
...
<cfdump var="#unplaced_placed#">

<CFLOOP QUERY="unplaced_placed">
<!--- Add item to graph --->
#unplaced# #result#  <br />
</CFLOOP>
...
<CFLOOP QUERY="unplaced_placed">
<!--- Add item to graph --->
<CFGRAPHDATA ITEM="#unplaced#" VALUE=#result#>
</CFLOOP>
</CFGRAPH>

<cfloop query="unplaced_placed"><cfset get_total = get_total + result></cfloop>&nbsp; &nbsp; Total of &nbsp; #get_total# &nbsp; Students</th></tr>
<tr><td align="center">
<cfgraph query="unplaced_placed" type="pie" valueColumn="result" itemColumn="unplaced" graphHeight="200" graphWidth="300"></cfgraph>
</td></tr>
</table><br>
---->
<!--- TOTAL ASIANS X NON-ASIANS --->
<cfquery name="asians_non_asians" datasource="caseusa">
SELECT 'Non-Asians', count( studentid ) AS result 
FROM smg_students
INNER JOIN smg_countrylist  ON countryresident = countryid
WHERE active = '1' and continent != 'asia' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
UNION 
...
<cfquery name="asians_statistics" datasource="caseusa">
SELECT 'Unplaced', count( studentid ) AS result 		
FROM smg_students
INNER JOIN smg_countrylist  ON countryresident = countryid
WHERE active = '1' and continent = 'asia' and hostid = '0' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
OR	active = '1' and continent = 'asia' and hostid != '0' AND `host_fam_approved` > '4' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
UNION 
SELECT 'Placed', count( studentid ) AS result 		
FROM smg_students
INNER JOIN smg_countrylist  ON countryresident = countryid
WHERE active = '1' and continent = 'asia' and hostid != '0' AND `host_fam_approved` < '5' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
</cfquery>
<table align="center" width="90%" cellpadding=6 cellspacing="0" frame="box"><br>
...
<cfquery name="asian_male_statistics" datasource="caseusa">
SELECT 'M-Unplaced', count( studentid ) AS result
FROM smg_students
INNER JOIN smg_countrylist ON countryresident = countryid
WHERE active = '1' AND continent = 'asia' AND hostid = '0' AND sex = 'male' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
OR active = '1' AND continent = 'asia' AND hostid != '0' AND sex = 'male' AND `host_fam_approved` > '5' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
UNION
SELECT 'M-Placed', count( studentid ) AS result
FROM smg_students
INNER JOIN smg_countrylist ON countryresident = countryid 
WHERE active = '1' AND continent = 'asia' AND hostid != '0' AND sex = 'male' AND `host_fam_approved` < '5' AND onhold_approved <= '4'
...
<cfquery name="asian_female_statistics" datasource="caseusa">
SELECT 'F-Unplaced', count( studentid ) AS result
FROM smg_students
INNER JOIN smg_countrylist ON countryresident = countryid
WHERE active = '1' AND continent = 'asia' AND hostid = '0' AND sex = 'female'  AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
OR  active = '1' AND continent = 'asia' AND hostid != '0' AND sex = 'female'  AND `host_fam_approved` > '5' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
UNION
SELECT 'F-Placed', count( studentid ) AS result
FROM smg_students
INNER JOIN smg_countrylist ON countryresident = countryid
WHERE active = '1' AND continent = 'asia' AND hostid != '0' AND sex = 'female' AND `host_fam_approved` < '5' AND onhold_approved <= '4'
...
<cfquery name="non_asians_unplaced_placed" datasource="caseusa">
SELECT 'Unplaced', count( studentid ) AS result 
FROM smg_students
INNER JOIN smg_countrylist  ON countryresident = countryid
WHERE active = '1' and continent != 'asia' and hostid = '0' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
OR active = '1' and continent != 'asia' and hostid != '0' AND `host_fam_approved` > '5' AND onhold_approved <= '4'
	  AND ( <cfloop list="#form.programid#" index="prog"> programid = #prog# <cfif prog is #ListLast(form.programid)#><Cfelse>or</cfif> </cfloop> )
UNION 
SELECT 'Placed', count( studentid ) AS result 
FROM smg_students
...
<cfquery name="non_asian_male_statistics" datasource="caseusa">
SELECT 'M-Unplaced', count( studentid ) AS result
FROM smg_students
INNER JOIN smg_countrylist ON countryresident = countryid
WHERE active = '1' AND continent != 'asia' AND hostid = '0' AND sex = 'male' AND onhold_approved <= '4'
...
<cfquery name="non_asian_female_statistics" datasource="caseusa">
SELECT 'F-Unplaced', count( studentid ) AS result
FROM smg_students
INNER JOIN smg_countrylist ON countryresident = countryid
WHERE active = '1' AND continent != 'asia' AND hostid = '0' AND sex = 'female' AND onhold_approved <= '4'
