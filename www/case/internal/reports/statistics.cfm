...
<cfquery name="get_stu_placed" datasource="caseusa">
	...
	WHERE hostid != '0' 
		...
		AND canceldate IS NULL
		<cfif form.date_host_fam_approved NEQ ''>
...
<tr><td align="left">
	...
	<cfif form.date_host_fam_approved NEQ ''>&nbsp &nbsp Students placed by <b>#DateFormat(form.date_host_fam_approved, 'mm/dd/yyyy')#</b><br></cfif>
	&nbsp &nbsp Students in Program(s) : &nbsp #get_stu_in_prog.Count_stu#<br>
...
<cfquery name="get_stu_assigned_region" datasource="caseusa">
...
<cfquery name="get_stu_placed_region" datasource="caseusa">
...
<cfquery name="get_stu_unassigned" datasource="caseusa">
	SELECT	SUM(if(not isnull(StudentID), 1, 0) ) AS assigned_students,
			SUM(IF(SEX = 'FEMALE', 1, 0)) AS assigned_female,
			SUM(IF(SEX = 'MALE', 1, 0)) AS assigned_male
	FROM 	smg_students
...
<cfquery name="get_unassigned_placed" datasource="caseusa">
	SELECT	SUM(if(not isnull(StudentID), 1, 0) ) as placed_student,
			SUM(if(Sex = 'Female', 1, 0)) as placed_female,
			SUM(if(Sex = 'Male', 1, 0)) as placed_male
	FROM 	smg_students
