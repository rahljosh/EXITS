...
<!--- **** COPIED FROM DOCUMENT_TRACKING.CFM - NEEDS TO BE CHANGED **** --->
...
<cfquery name="get_total_students" datasource="caseusa">
	...
	FROM 	smg_students s
	INNER JOIN smg_hostdocshistory hist ON s.studentid = hist.studentid
	INNER JOIN smg_hostdocshistory doc_hist ON hist.historyid = doc_hist.historyid
	WHERE companyid = #client.companyid# 
		...
		<cfif client.usertype is '6'>
			...
			'#i#' <cfif #ListLast(ad_users)# is #i#><cfelse> or hist.placerepid = </cfif> </Cfloop>)
		</cfif>							
</cfquery>
