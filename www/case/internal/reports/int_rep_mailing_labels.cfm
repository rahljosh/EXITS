	...
	<body>
		...
		<div class="Section1">
			...
			<cfquery name="get_reps" datasource="caseusa"> 
				SELECT DISTINCT	u.businessname, u.firstname, u.lastname, u.address, u.address2, u.city, u.state, u.zip, u.country, smg_countrylist.countrycode
				FROM smg_users u
				LEFT JOIN smg_countrylist on smg_countrylist.countryid = u.country
				WHERE u.usertype = 8
					<cfif NOT IsDefined('form.inactive')>AND u.active = 1</cfif>
			...
			<cfoutput query="get_reps">
				...
				<td class="label">
					<p>#businessname#</p>
					<p><cfif address is ''>#address2#<cfelse>#address#</cfif></p>
					<p>#city# <cfif state neq 0>#state#</cfif> #countrycode# #zip#</p>
				</td>
