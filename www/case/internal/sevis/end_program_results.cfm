...
<cfelse>
	...
	<cffile action="read" file="/var/www/html/student-management/nsmg/sevis/xml/#get_company.companyshort#/activate/results/#form.filename#" variable="myxml">
	...
	<cfloop index="i" from = "1" to = #numItems#>
		...
		<cfif #mydoc.TransactionLog.BatchDetail.Process.RECORD[i].Result.XmlAttributes.Status# is 'true'>
			...
			<cfquery name="update_active_batch" datasource="caseusa">
				UPDATE smg_students
		...
		<cfelse> <!--- ERROR - STUDENT WAS NOT ACTIVATED --->
			...
			<cfif find("status type is ACTIVE",reason) is '0'> <!--- non active errors --->
				<cfquery name="update_ds2019" datasource="caseusa"> <!--- UPDATE SEVIS ACTIVATED TO 0 --->
					UPDATE smg_students
