...
<cfif NOT IsDefined('form.programid')>
	You must select a program in order to run this report. Please try again.
	<cfabort>
</cfif>
...
<table align="center" width="90%" frame="below">
	...
	<tr><td>Program(s) included in this report: : <br>
	<cfloop query="get_programs">&nbsp &nbsp <b>(#ProgramID#) &nbsp #programname#</b><br></cfloop></td></tr>
	...
	<tr><td>
		<table align="center" width="100%">
			<cfloop query="get_hosts">
				<!--- host father --->
				<cfquery name="host_father" datasource="MySql">
				...
				<!--- host mother --->
				<cfquery name="host_mother" datasource="MySql">
					SELECT cbcfamid, hostid
					FROM smg_hosts_cbc
					WHERE hostid = <cfqueryparam value="#get_hosts.hostid#" cfsqltype="cf_sql_integer">
						AND cbc_type = 'mother'
